...
echo "🚀 Starting Christian Faith Web Local Development Environment..."
...
if docker compose ps | grep -q "christian-faith-web-minio-local"; then
    ...
    if ! docker compose exec -T minio mc ls local/christianfaith-local >/dev/null 2>&1; then
        docker compose exec -T minio mc mb local/christianfaith-local >/dev/null 2>&1
        docker compose exec -T minio mc anonymous set download local/christianfaith-local >/dev/null 2>&1
        echo -e "${GREEN}✅ MinIO bucket 'christianfaith-local' created${NC}"
    else
        echo -e "${GREEN}✅ MinIO bucket 'christianfaith-local' already exists${NC}"
    fi
fi
...
echo -e "   MailHog: ${GREEN}http://localhost:${DOCKER_MAILHOG_WEB_PORT:-5105}${NC}"
echo -e "   Redis Commander: ${GREEN}http://localhost:${DOCKER_REDIS_COMMANDER_PORT:-5106}${NC}"
if docker compose ps | grep -q "christian-faith-web-minio-local"; then
    echo -e "   MinIO Console: ${GREEN}http://localhost:${DOCKER_MINIO_CONSOLE_PORT:-5108}${NC} (minioadmin/minioadmin)"
fi
echo -e ""
...
if docker compose ps | grep -q "christian-faith-web-minio-local"; then
    echo -e "   MinIO S3 API: ${GREEN}localhost:${DOCKER_MINIO_PORT:-5107}${NC}"
fi
