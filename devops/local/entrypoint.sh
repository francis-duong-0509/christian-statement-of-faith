...
if [ ! -z "$REDIS_HOST" ]; then
    wait_for_service "Redis" "redis-cli -h $REDIS_HOST ping"
fi

# Wait for MinIO (S3-compatible storage)
if [ ! -z "$AWS_ENDPOINT" ] && [ "$FILESYSTEM_DISK" = "s3" ]; then
    # Extract hostname from endpoint URL
    MINIO_HOST=$(echo "$AWS_ENDPOINT" | sed -e 's|^http://||' -e 's|^https://||' -e 's|:.*||')
    MINIO_PORT=$(echo "$AWS_ENDPOINT" | sed -e 's|.*:||' -e 's|/.*||')
    wait_for_service "MinIO" "nc -z $MINIO_HOST $MINIO_PORT"
fi

# Wait for MailHog (if configured as mail host)
if [ "$MAIL_HOST" = "christian-faith-web-mailhog-local" ]; then
...
# Create MinIO bucket if using S3 filesystem
if [ "$FILESYSTEM_DISK" = "s3" ] && [ ! -z "$AWS_BUCKET" ]; then
    ...
    mc alias set local $AWS_ENDPOINT $AWS_ACCESS_KEY_ID $AWS_SECRET_ACCESS_KEY >/dev/null 2>&1 || true

    # Create bucket if it doesn't exist
    if ! mc ls local/$AWS_BUCKET >/dev/null 2>&1; then
        echo "🪣 Creating MinIO bucket: $AWS_BUCKET"
        mc mb local/$AWS_BUCKET >/dev/null 2>&1 || echo "⚠️  Could not create bucket, will continue..."
    else
        echo "✅ MinIO bucket already exists: $AWS_BUCKET"
    fi
fi
...
# Generate JWT secret if not set (commented out - JWT package may not be installed)
...
if ! php artisan tinker --execute="Redis::ping();" 2>/dev/null; then
    ...
    echo "   🔧 Check REDIS_PORT: $REDIS_PORT"
    echo "   🛑 Stopping all processes due to Redis connection failure"
    exit 1
fi
echo "   ✅ Redis connection successful"

# Storage/S3 connection (if configured)
if [ "$FILESYSTEM_DISK" = "s3" ]; then
    echo "   🪣 S3/MinIO connection:"
    if ! php artisan tinker --execute="Storage::disk('s3')->exists('test');" 2>/dev/null; then
        echo "   ❌ S3 connection failed"
        echo "   🔧 Check AWS_ENDPOINT: $AWS_ENDPOINT"
        echo "   🔧 Check AWS_ACCESS_KEY_ID: $AWS_ACCESS_KEY_ID"
        echo "   🔧 Check AWS_BUCKET: $AWS_BUCKET"
        echo "   🛑 Stopping all processes due to S3/MinIO connection failure"
        exit 1
    fi
    echo "   ✅ S3/MinIO connection successful"
fi
